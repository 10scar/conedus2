...
import java.util.NoSuchElementException;
    ...
    public void addInOrder(E elemento) {
        if (size == capacity) {
            grow();
        }
    ...
    public void printArray() {
        ...
        private int busquedaBinaria(E elemento, int inferior, int superior) {
            if (inferior > superior) {
                return inferior;
            }
            int mid = (inferior + superior) / 2;
            if (data[mid] == null || elemento.compareTo(data[mid]) <= 0) {
                return binarySearch(elemento, inferior, mid - 1);
            } else {
                return binarySearch(elemento, mid + 1, superior);
            }
        }
    }
